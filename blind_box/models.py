...
class BlindBasic(models.Model):
    ...
    open_buy_times = models.BooleanField('是否开启购买转盘次数', default=True)
...
class WinningRecordAbstract(models.Model):
    ...
    def set_received(self, address):
        ...
        if self.status == self.ST_PENDING_RECEIVE:
            ...
            self.express_phone = address.phone
            self.save(update_fields=['status', 'receive_at', 'express_address', 'express_user_name', 'express_phone'])
...
class LotteryPurchaseRecord(models.Model):
    ...
    amount = models.DecimalField('金额', max_digits=13, decimal_places=2)
    actual_amount = models.DecimalField('实付金额', max_digits=13, decimal_places=2, default=0)
    refund_amount = models.DecimalField('已退款金额', max_digits=10, decimal_places=2, default=0)
    status = models.PositiveSmallIntegerField('状态', choices=STATUS_CHOICES, default=ST_UNPAID)
