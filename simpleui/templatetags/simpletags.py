...
def menus(context, _get_config=None):
    ...
    key = 'system_keep'
    perms = {'add': True, 'change': False, 'delete': True, 'view': False}
    if config and 'menus' in config:
        if config.get(key, None):
            total = config.get('menus')
            user_permissions = list(context.request.user.get_all_permissions())
            for level_one_menu in total:
                # 与一级菜单codename比对权限
                for apps in app_list:
                    if level_one_menu.get('app') == apps['app_label']:
                        ...
                        if context.request.user.is_superuser:
                            ...
                            continue
                              ...
                              or (level_one_menu['codename'] + '.' + level_two_menu[
                                ...
                                (level_one_menu['codename'] + '.view_' + level_two_menu[
                                    'codename'] not in user_permissions) \
                                and (
                                level_one_menu['codename'] + '.add_' + level_two_menu[
                            'codename'] not in user_permissions) \
    ...
    for app in app_list:
        _models = [
            ...
            for m in app.get('models') if
            (config.get('hide_menu') and m.get('name') not in config['hide_menu']) or not config.get(
                'hide_menu')] if app.get(
            'models') else []
