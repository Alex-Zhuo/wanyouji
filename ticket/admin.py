...
from dj_ext.permissions import RemoveDeleteModelAdmin, RemoveDeleteTabularInline, OnlyViewAdmin, ChangeAndViewAdmin, \
    OnlyReadTabularInline, SaveSignalAdmin, RemoveDeleteStackedInline, OnlyReadStackedInline
...
class TicketOrderAdmin(AjaxAdmin, ChangeAndViewAdmin):
    ...
    list_filter = (
        ...
        'order_type', 'channel_type', 'discount_type', 'wx_pay_config', AgentFilter, 'is_paper', 'express_status',
        'deliver_at')
    ...
    def set_refund(self, request, queryset):
        user = request.user
        from caches import get_redis
        redis = get_redis()
    ...
    set_refund.short_description = '申请退款'
    set_refund.type = 'success'
    set_refund.icon = 'el-icon-s-promotion'
    # 指定为弹出层，这个参数最关键
    set_refund.layer = {
        # 弹出层中的输入框配置
        # 这里指定对话框的标题
