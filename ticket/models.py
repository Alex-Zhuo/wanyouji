...
class SessionInfo(UseNoAbstract):
    ...
    is_dy_code = models.BooleanField('是否动态码', default=False, help_text='勾选后需要填写动态有效时间才能有效,大于2小时才能分享核销码图片')
    dc_expires_in = models.PositiveIntegerField('动态码有效时间', default=30, help_text='单位：分钟')
    create_at = models.DateTimeField('创建时间', auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True, verbose_name='更新时间')
