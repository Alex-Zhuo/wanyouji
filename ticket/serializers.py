...
from common.utils import s_id_card, s_name, s_mobile
...
class TicketOrderSerializer(serializers.ModelSerializer):
    ...
    def get_name(self, obj):
        from common.utils import s_name
        return s_name(obj.name) if obj.name else None

    def get_mobile(self, obj):
        from common.utils import s_mobile
    ...
    class Meta:
        model = TicketOrder
        fields = ['id', 'title', 'name', 'mobile', 'id_card', 'express_address', 'order_no', 'tiktok_order_id',
                  'ks_order_no',
...
class TicketOrderDetailNewSerializer(TicketOrderDetailSerializer):
    ...
    class Meta:
        model = TicketOrder
        fields = TicketOrderDetailSerializer.Meta.fields
...
class TicketOrderCreateCommonSerializer(serializers.ModelSerializer):
    ...
    def after_create(self, order, show_type=None):
        ...
        if order.id_card:
            TicketOrder.get_or_set_real_name_buy_num(order.session.id, order.id_card, order.multiply, is_get=False)
...
class TicketOrderCreateNewSerializer(TicketOrderCreateCommonSerializer):
    @atomic
    def create(self, validated_data):
        # ticket_list [dict(level_id=1,seat_id=***)]
        request = self.context.get('request')
        ...
        pay_end_at = inst.get_end_at()
        self.after_create(inst, show_type)
...
class TicketOrderOnSeatCreateNewSerializer(TicketOrderCreateCommonSerializer):
    @atomic
    def create(self, validated_data):
        # ticket_list [dict(level_id=1,multiply=2)]
        request = self.context.get('request')
